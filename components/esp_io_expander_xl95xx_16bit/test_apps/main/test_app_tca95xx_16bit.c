...
static const char *TAG = "xl95xx_16bit test";
...
static void i2c_dev_xl95xx_16bit_init(void)
{
    esp_err_t ret = esp_io_expander_new_i2c_xl95xx_16bit(i2c_handle, I2C_ADDRESS, &io_expander);
    TEST_ASSERT_EQUAL_MESSAGE(ESP_OK, ret, "TCA9554 create returned error");
}

static void i2c_dev_xl95xx_16bit_deinit(void)
{
    esp_err_t ret = esp_io_expander_del(io_expander);
    TEST_ASSERT_EQUAL_MESSAGE(ESP_OK, ret, "TCA9554 delete returned error");
}

TEST_CASE("IO expander xl95xx_16bit test", "[xl95xx_16bit]")
{
    i2c_bus_init();
    i2c_dev_xl95xx_16bit_init();
